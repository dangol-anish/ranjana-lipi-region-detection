...
try:
    from .normalize import NormalizationError, normalize_image
except ImportError:
...
def save_normalized_png(image_path: Path, output_path: Path) -> None:
...
def process_class(data_dir: Path, output_root: Path, class_name: str) -> tuple[int, int]:
    ...
    for source_path in image_files(class_dir):
        ...
        try:
            save_normalized_png(source_path, output_path)
            processed += 1
    ...
    try:
        save_normalized_png(reference_path, reference_output)
...
def main() -> None:
    ...
    for class_name in SELECTED_CLASSES:
        processed, skipped = process_class(data_dir, output_root, class_name)
